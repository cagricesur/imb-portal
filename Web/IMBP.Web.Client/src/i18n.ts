...
import { getTranslation, type TranslationData } from "@imb-portal/api";
...
i18n
  ...
  .init<HttpBackendOptions>({
    ...
    detection: {
      ...
      lookupCookie: PortalContants.CookieKeys.Language,
      caches: ["cookie"],
    ...
    backend: {
      ...
      request(_options, language, payload, callback) {
        if (payload) {
          ...
          api
            .postApiTranslationAddMissing(translations)
            .then(() => {})
            .catch(() => {});
        } else {
          api
            .getApiTranslationList({ language })
            .then((data) => {
